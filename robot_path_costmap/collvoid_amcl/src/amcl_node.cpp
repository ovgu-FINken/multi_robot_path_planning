...
#include "tf2/LinearMath/Transform.h" // depr.: "tf/transform_broadcaster.h", "tf/transform_listener.h"
#include "tf2/convert.h"
#include "tf2/utils.h"
#include "tf2_geometry_msgs/tf2_geometry_msgs.h"
#include "tf2_ros/buffer.h"
#include <tf2/exceptions.h>
#include "tf2_ros/message_filter.h" // depr.:  "tf/message_filter.h"
...
#include "collvoid_msgs/PoseArrayWeighted.h" // COLLVOID
                       ...
                       first_reconfigure_call_(true)
{
  ...
  particlecloud_weighted_pub_ = nh_.advertise<collvoid_msgs::PoseArrayWeighted>("particlecloud_weighted", 1, true); //COLLVOID
...
void AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr &laser_scan)
{
  ...
  if (resampled || force_publication)
  {
    ...
    if (max_weight > 0.0 && false)
    {
      ...
      cloud_msg2.header.stamp = laser_scan->header.stamp;
      cloud_msg2.header.frame_id = global_frame_id_;
      cloud_msg2.poses.resize(hyps[max_weight_hyp].count);   // resize(set->sample_count)
      cloud_msg2.weights.resize(hyps[max_weight_hyp].count); // resize(set->sample_count)
      int count = 0;
      for (int i = 0; i < set2->sample_count; i++)
      {
        if (pf_kdtree_get_cluster(set2->kdtree, set2->samples[i].pose) == max_weight_hyp)
        {
          if (count > hyps[max_weight_hyp].count)
          {
            ROS_WARN("SOMETHING is WRONG");
            break;
          }

          cloud_msg2.poses[i].position.x = set2->samples[i].pose.v[0];
          cloud_msg2.poses[i].position.y = set2->samples[i].pose.v[1];
          cloud_msg2.poses[i].position.z = 0;
          tf2::Quaternion q;
          q.setRPY(0, 0, set2->samples[i].pose.v[2]);
          tf2::convert(q, cloud_msg2.poses[i].orientation);
          ...
          //                cloud_msg2.poses.at(count));

          cloud_msg2.weights[count] = set2->samples[i].weight / hyps[max_weight_hyp].weight;
          count++;
        }
      }
